...
data <- read.dta13("C:/Users/T_doma/Desktop/Data diplomka/Cat_1/Category_1.dta")
cols<-ncol(data)
  rows<-nrow(data)
  ...
  treat<- "CZE_SWE"
  control_origins<-c("SVK","HUN","SVN","POL","ROM","HRV","LTU")
  control_destinations<-c("SWE","DNK","NOR","FIN","NLD","BEL","IRL","AUT")
  conditions_1 <-eval(parse(text=paste("data[,2]=='", control_origins,"'",sep="", collapse=" | ")))
  conditions_2 <-eval(parse(text=paste("data[,3]=='", control_destinations,"'",sep="", collapse=" | ")))
  control_num<- length(control_origins)*length(control_destinations)
  controls<-unique(data[conditions_1 & conditions_2,1])
  ...
  load("C:/Users/Teich/Desktop/Data diplomka/Cat_1/Synth_C1_nolag_inst_results.RData")
  pairs<- c("CZE_SVK","CZE_ITA","CZE_DEU","CZE_POL","CZE_GBR","CZE_HUN","CZE_FRA","CZE_SWE","CZE_AUT","CZE_BEL")
  destinations<-c("Slovakia", "Italy", "Germany","Poland","United Kingdom","Hungary","France","sweden","Austria","Belgium")
