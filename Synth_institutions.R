...
library(readstata13)
  ...
  data <- read.dta13("C:/Users/Teich/Desktop/Data diplomka/Total/Total.dta")
  cols<-ncol(data)
  rows<-nrow(data)
  data <- cbind(data,as.numeric(as.factor(data[,2])))
  data <- cbind(data,as.numeric(as.factor(data[,1])))
  pred<-c(which(colnames(data) == "GDP"),which(colnames(data) == "Population"),which(colnames(data) == "FMA"),which(colnames(data) == "Country_risk"),which(colnames(data) == "FDI_share"),which(colnames(data) == "RER"),which(colnames(data) == "HICP"),which(colnames(data) == "Unemployment"),which(colnames(data) == "Gov_size"),which(colnames(data) == "Legal_sys"),which(colnames(data) == "Sound_money"),which(colnames(data) == "Trade_freedom"),which(colnames(data) == "Regulation"),which(colnames(data) == "Broadband_subs"),which(colnames(data) == "Internet_share"),which(colnames(data) == "Tel_subscr"))
...
#  }
  ...
  load("C:/Users/Teich/Desktop/Data diplomka/Total/Synth_institutions_results.RData")
  ticks<-seq(from = 4,by=8,length.out=6)
  axis<- c("2005Q4","2007Q4","2009Q4","2011Q4","2013Q4","2015Q4")
  model_version<-"institutions"
  ...
  expl_var_w_export<-rbind(synt_res_0$solution.v, synt_res_1$solution.v, synt_res_2$solution.v, synt_res_3$solution.v, synt_res_4$solution.v, synt_res_5$solution.v, synt_res_6$solution.v, synt_res_7$solution.v, synt_res_8$solution.v, synt_res_9$solution.v)
  rownames(expl_var_w_export)<-c(0:9)
  write.csv(t(expl_var_w_export), file = paste("C:/Users/Teich/Desktop/Data diplomka/Total/expl_var_w_export_",model_version,".csv",sep=""))
  
  donor_w_export<-cbind(donor_w_0, donor_w_1, donor_w_2, donor_w_3, donor_w_4, donor_w_5, donor_w_6, donor_w_7, donor_w_8, donor_w_9)
  colnames(donor_w_export)<-c(0:9)
  write.csv(donor_w_export, file = paste("C:/Users/Teich/Desktop/Data diplomka/Total/donor_w_export_",model_version,".csv",sep=""))