...
data <- read.dta13("C:/Users/Teich/Desktop/Data diplomka/Cat_7/Category_7.dta")
cols<-ncol(data)
rows<-nrow(data)
...
df_results_7<-list() 
gap_7<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_7))
colnames(gap_7)<-new_cont_7
l<-0
for (i in new_cont_7){
  l<-l+1
  d_pre<-dataprep(foo = data, predictors = pred,
                  dependent = dep, unit.variable = unit_num,
                  time.variable = time, treatment.identifier = i,
                  controls.identifier = c(treat,new_cont_7[new_cont_7!=i]), time.predictors.prior = pretreat,
                  time.optimize.ssr = pretreat, time.plot = plot_per,
                  unit.names.variable = unit_name)
...
gap_7_red<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_7))
colnames(gap_7_red)<-new_cont_7
for (i in new_cont_7){
...
data_prepared_7_time_sens<-dataprep(foo = data, predictors = pred,
                                    dependent = dep, unit.variable = unit_num,
                                    time.variable = time, treatment.identifier = treat,
                                    controls.identifier = new_cont_7, time.predictors.prior = pretreat_alt,
                                    time.optimize.ssr = pretreat_alt, time.plot = plot_per,
                                    unit.names.variable = unit_name)
...
df_results_8<-list() 
gap_8<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_8))
colnames(gap_8)<-new_cont_8
l<-0
for (i in new_cont_8){
  l<-l+1
  d_pre<-dataprep(foo = data, predictors = pred,
                  dependent = dep, unit.variable = unit_num,
                  time.variable = time, treatment.identifier = i,
                  controls.identifier = c(treat,new_cont_8[new_cont_8!=i]), time.predictors.prior = pretreat,
                  time.optimize.ssr = pretreat, time.plot = plot_per,
                  unit.names.variable = unit_name)
...
gap_8_red<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_8))
colnames(gap_8_red)<-new_cont_8
for (i in new_cont_8){
...
data_prepared_8_time_sens<-dataprep(foo = data, predictors = pred,
                                    dependent = dep, unit.variable = unit_num,
                                    time.variable = time, treatment.identifier = treat,
                                    controls.identifier = new_cont_8, time.predictors.prior = pretreat_alt,
                                    time.optimize.ssr = pretreat_alt, time.plot = plot_per,
                                    unit.names.variable = unit_name)
...
df_results_9<-list() 
gap_9<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_9))
colnames(gap_9)<-new_cont_9
l<-0
for (i in new_cont_9){
  l<-l+1
  d_pre<-dataprep(foo = data, predictors = pred,
                  dependent = dep, unit.variable = unit_num,
                  time.variable = time, treatment.identifier = i,
                  controls.identifier = c(treat,new_cont_9[new_cont_9!=i]), time.predictors.prior = pretreat,
                  time.optimize.ssr = pretreat, time.plot = plot_per,
                  unit.names.variable = unit_name)
...
gap_9_red<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_9))
colnames(gap_9_red)<-new_cont_9
for (i in new_cont_9){
...
data_prepared_9_time_sens<-dataprep(foo = data, predictors = pred,
                                    dependent = dep, unit.variable = unit_num,
                                    time.variable = time, treatment.identifier = treat,
                                    controls.identifier = new_cont_9, time.predictors.prior = pretreat_alt,
                                    time.optimize.ssr = pretreat_alt, time.plot = plot_per,
                                    unit.names.variable = unit_name)
...
df_results_10<-list() 
gap_10<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_10))
colnames(gap_10)<-new_cont_10
l<-0
for (i in new_cont_10){
  l<-l+1
  d_pre<-dataprep(foo = data, predictors = pred,
                  dependent = dep, unit.variable = unit_num,
                  time.variable = time, treatment.identifier = i,
                  controls.identifier = c(treat,new_cont_10[new_cont_10!=i]), time.predictors.prior = pretreat,
                  time.optimize.ssr = pretreat, time.plot = plot_per,
                  unit.names.variable = unit_name)
...
gap_10_red<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_10))
colnames(gap_10_red)<-new_cont_10
for (i in new_cont_10){
...
data_prepared_10_time_sens<-dataprep(foo = data, predictors = pred,
                                     dependent = dep, unit.variable = unit_num,
                                     time.variable = time, treatment.identifier = treat,
                                     controls.identifier = new_cont_10, time.predictors.prior = pretreat_alt,
                                     time.optimize.ssr = pretreat_alt, time.plot = plot_per,
                                     unit.names.variable = unit_name)
...
dist_mat<-dist_mat[order(dist_mat, na.last=TRUE, decreasing=FALSE)[1:50]]
new_cont_11<-names(dist_mat)

data_prepared_11<-dataprep(foo = data, predictors = pred,
                           dependent = dep, unit.variable = unit_num,
                           time.variable = time, treatment.identifier = treat,
                           controls.identifier = new_cont_11, time.predictors.prior = pretreat,
                           time.optimize.ssr = pretreat, time.plot = plot_per,
                           unit.names.variable = unit_name)

synt_res_11<-synth(data.prep.obj = data_prepared_11,verbose=FALSE)
donor_w_11 <-synt_res_11$solution.w
rownames(donor_w_11)<- new_cont_11
tab_res_11<-cbind(data_prepared_11$Y1plot,data_prepared_11$Y0plot %*% synt_res_11$solution.w,data_prepared_11$Y1plot - (data_prepared_11$Y0plot %*% synt_res_11$solution.w))
rownames(tab_res_11)<-optimizer_per
colnames(tab_res_11)<-c("Treated","Synthetic","Gap")

df_results_11<-list() 
gap_11<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_11))
colnames(gap_11)<-new_cont_11
l<-0
for (i in new_cont_11){
  l<-l+1
  d_pre<-dataprep(foo = data, predictors = pred,
                  dependent = dep, unit.variable = unit_num,
                  time.variable = time, treatment.identifier = i,
                  controls.identifier = c(treat,new_cont_11[new_cont_11!=i]), time.predictors.prior = pretreat,
                  time.optimize.ssr = pretreat, time.plot = plot_per,
                  unit.names.variable = unit_name)
  df_results_11[[i]]<-synth(data.prep.obj = d_pre,verbose=FALSE)
  gap_11[,l] <- d_pre$Y1plot - (d_pre$Y0plot %*% df_results_11[[i]]$solution.w)
  
}
gap_treat_11 <- data_prepared_11$Y1plot - (data_prepared_11$Y0plot %*% synt_res_11$solution.w)

gap_11_red<-matrix(0,nrow=length(optimizer_per),ncol=length(new_cont_11))
colnames(gap_11_red)<-new_cont_11
for (i in new_cont_11){
  if(df_results_11[[i]]$loss.v<5*synt_res_11$loss.v){
    gap_11_red[,i]<-gap_11[,i]}
}

data_prepared_11_time_sens<-dataprep(foo = data, predictors = pred,
                                     dependent = dep, unit.variable = unit_num,
                                     time.variable = time, treatment.identifier = treat,
                                     controls.identifier = new_cont_11, time.predictors.prior = pretreat_alt,
                                     time.optimize.ssr = pretreat_alt, time.plot = plot_per,
                                     unit.names.variable = unit_name)

synt_res_11_time_sens<-synth(data.prep.obj = data_prepared_11_time_sens,verbose = FALSE)

#Results

path.plot(dataprep.res = data_prepared_11,synth.res = synt_res_11,Main=treat)
abline(v="2012", col="black")
synt_res_11$solution.v
donor_w_11
tab_res_11

plot(gap_treat_11,type="l",col="black",lty=1,lwd=2,ylim=c(-2*max(abs(gap_treat_11)),2*max(abs(gap_treat_11))),main=treat)
for( i in 1:ncol(gap_11)) {
  lines(gap_11[,i],type="l",col="gray",lty=1,lwd=1)
}
lines(gap_treat_11,type="l",col="black",lty=1,lwd=2)
abline(v="2012", col="black")

plot(gap_treat_11,type="l",col="black",lty=1,lwd=2,ylim=c(-2*max(abs(gap_treat_11)),2*max(abs(gap_treat_11))),main=treat)
for( i in 1:ncol(gap_11_red)) {
  lines(gap_11_red[,i],type="l",col="gray",lty=1,lwd=1)
}
lines(gap_treat_11,type="l",col="black",lty=1,lwd=2)
abline(v="2012", col="black")

path.plot(dataprep.res = data_prepared_11_time_sens,synth.res = synt_res_11_time_sens,Main=treat)
abline(v="2012", col="black")
abline(v="2009", col="black",lty=2)



load("C:/Users/Teich/Desktop/Data diplomka/Cat_7/Synth_C7_ed50_results.RData")
pairs<- c("CZE_DEU","CZE_FRA","CZE_GBR","CZE_SVK","CZE_NLD","CZE_POL","CZE_ITA","CZE_AUT","CZE_ESP","CZE_BEL","CZE_HUN")
