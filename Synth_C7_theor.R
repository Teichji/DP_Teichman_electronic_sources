...
cols<-ncol(data)
  ...
  pred<-c(which(colnames(data) == "Dist"),which(colnames(data) == "GDP_o"),which(colnames(data) == "GDP_d"),which(colnames(data) == "RER"),which(colnames(data) == "Rem_o"),which(colnames(data) == "Rem_d"),which(colnames(data) == "contig"),which(colnames(data) == "comcur"),which(colnames(data) == "Institutions_o"),which(colnames(data) == "Institutions_d"),which(colnames(data) == "IT_NET_USER_ZS_o"),which(colnames(data) == "IT_NET_USER_ZS_d"),which(colnames(data) == "TEL_SETS_o"),which(colnames(data) == "TEL_SETS_d"))
  dep<-cols
  time<-4
  unit_name <- 1
  unit_num<-(cols+1)
  ...
  load("C:/Users/Teich/Desktop/Data diplomka/Cat_7/Synth_C7_theor_results.RData")
  ...
  ticks<-c(1,6,11,16)
  pairs_num<-c(2:7,9:length(pairs))
  cat_ver<- "7_Theor"
