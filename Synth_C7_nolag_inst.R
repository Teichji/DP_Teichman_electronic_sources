...
cols<-ncol(data)
  ...
  pred<-c(which(colnames(data) == "Dist"),which(colnames(data) == "GDP_o"),which(colnames(data) == "GDP_d"),which(colnames(data) == "GPD_PC_o"),which(colnames(data) == "GPD_PC_d"),which(colnames(data) == "Population_o"),which(colnames(data) == "Population_d"),which(colnames(data) == "RER"),which(colnames(data) == "Rem_o"),which(colnames(data) == "Rem_d"),which(colnames(data) == "contig"),which(colnames(data) == "comcur"),which(colnames(data) == "Institutions_o"),which(colnames(data) == "Institutions_d"),which(colnames(data) == "IT_NET_USER_ZS_o"),which(colnames(data) == "IT_NET_USER_ZS_d"),which(colnames(data) == "TEL_SETS_o"),which(colnames(data) == "TEL_SETS_d"))
  dep<-cols
  time<-4
  unit_name <- 1
  unit_num<-(cols+1)
  ...
  treat<- "CZE_ESP"
  control_origins<-c("SVK","HUN","SVN","POL","ROM","HRV","LTU")
  control_destinations<-c("ESP","ESP","FRA","DEU","GBR","PRT")
  conditions_1 <-eval(parse(text=paste("data[,2]=='", control_origins,"'",sep="", collapse=" | ")))
  conditions_2 <-eval(parse(text=paste("data[,3]=='", control_destinations,"'",sep="", collapse=" | ")))
  control_num<- length(control_origins)*length(control_destinations)
  controls<-unique(data[conditions_1 & conditions_2,1])
  ...
  df_results_11<-list() 
  gap_11<-matrix(0,nrow=length(optimizer_per),ncol=length(controls))
  colnames(gap_11)<-controls
  l<-0
  for (i in controls){
    l<-l+1
    d_pre<-dataprep(foo = data, predictors = pred,
  ...
  gap_11_red<-matrix(0,nrow=length(optimizer_per),ncol=length(controls))
  colnames(gap_11_red)<-controls
  for (i in controls){
  ...
  load("C:/Users/Teich/Desktop/Data diplomka/Cat_7/Synth_C7_nolag_inst_results.RData")
  pairs<- c("CZE_DEU","CZE_FRA","CZE_GBR","CZE_SVK","CZE_NLD","CZE_POL","CZE_ITA","CZE_AUT","CZE_ESP","CZE_BEL","CZE_HUN")
  destinations<-c("Germany","France","United Kingdom","Slovakia","Netherlands","Poland","Italy","Austria","Spain","Belgium","Hungary")
