...
data <- read.dta13("C:/Users/T_doma/Desktop/Data diplomka/Cat_2/Category_2.dta")
cols<-ncol(data)
  rows<-nrow(data)
  ...
  treat<- "CZE_SVN"
  control_origins<-c("SVK","HUN","SVN","POL","ROM","HRV","LTU")
  control_destinations<-c("SVN","SVK","ROM","HUN","HRV","BGR","LTU")
  conditions_1 <-eval(parse(text=paste("data[,2]=='", control_origins,"'",sep="", collapse=" | ")))
  conditions_2 <-eval(parse(text=paste("data[,3]=='", control_destinations,"'",sep="", collapse=" | ")))
  control_num<- length(control_origins)*length(control_destinations)
  controls<-unique(data[conditions_1 & conditions_2,1])
  ...
  load("C:/Users/Teich/Desktop/Data diplomka/Cat_2/Synth_C2_results.RData")
  pairs<- c("CZE_DEU","CZE_AUT","CZE_POL","CZE_ITA","CZE_SVK","CZE_FRA")
  destinations<-c("Germany","Austria","Poland","Italy","Slovakia","France")
